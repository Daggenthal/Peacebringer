...
fullBackup() {
    ...
    cd /tmp/Backup && sudo mariadb-dump --user=$userName --password=$passWord --lock-tables --all-databases > server_db_backup.sql
...
dbBackup() {
    ...
    cd /tmp/ && sudo mariadb-dump --user=$userName --password=$passWord --lock-tables --all-databases > $(date '+%Y-%m-%d').sql
